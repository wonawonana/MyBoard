...
public class Dialog_Input_favorite extends androidx.fragment.app.DialogFragment implements View.OnClickListener {
    ...
    public Dialog_Input_favorite(int cafe_i){
        i=cafe_i;
    }
    ...
    public void addDB(float newGameNum, float newClean, float newService){
        ...
        PostRef
                .document(MapView.mDataset_marker.get(i).id)
                .get()
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (document.exists()) {
                                ...
                                DocumentReference docRef=db.collection("cafe").document(MapView.mDataset_marker.get(i).id);
                                ...
                                cafeDB cafe=new cafeDB(MapView.mDataset_marker.get(i).place_name,newClean,
                                        newGameNum, newService,1, new ArrayList<>(), "", ""
                                        ,MapView.mDataset_marker.get(i).address_name,
                                        MapView.mDataset_marker.get(i).phone);
                                db.collection("cafe").document(MapView.mDataset_marker.get(i).id).set(cafe);
                                //좋아요 field 추가
